...
export function calculatePointToPolygonDistance(point: Vector, vertices: Vector[]): number {
    ...
    for (let i = 0; i < vertices.length; i++) {
        ...
        const r: number = Vector.dot(p2.sub(p1), point.sub(p1)) / Math.pow(p2.sub(p1).magnitude(), 2);
